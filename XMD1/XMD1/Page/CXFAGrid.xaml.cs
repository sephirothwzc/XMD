...
namespace XMD1.Page
{
    ...
    public partial class CXFAGrid : ContentPage
    {
        ...
        protected override void OnAppearing()
        {
            ...
            var qp = JsonConvert.DeserializeObject<List<QP_QUERYRESULT>>(Json);
            qp.AddRange(JsonConvert.DeserializeObject<List<QP_QUERYRESULT>>(Json));
            qp.AddRange(JsonConvert.DeserializeObject<List<QP_QUERYRESULT>>(Json));
            qp.AddRange(JsonConvert.DeserializeObject<List<QP_QUERYRESULT>>(Json));
            qp.AddRange(JsonConvert.DeserializeObject<List<QP_QUERYRESULT>>(Json));
            qp.AddRange(JsonConvert.DeserializeObject<List<QP_QUERYRESULT>>(Json));

            this.grid.Columns.Add(new TextColumn()
            {
                FieldName = "ATTRIBUTE_CODE",
                Caption = "ATTRIBUTE_CODE",
            });
            this.grid.Columns.Add(new TextColumn()
            {
                FieldName = "PAGE_REGISTER_GUID",
                Caption = "PAGE_REGISTER_GUID",
            });
            this.grid.Columns.Add(new TextColumn()
            {
                FieldName = "PAGE_REGISTER_GUID",
